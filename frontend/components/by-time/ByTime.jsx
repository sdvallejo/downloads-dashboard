...
/**
* ByTime component: info about downloads by time of the day
*/
class ByTime extends Component {
  ...
  getDownloads() {
    let url = this.props.backendurl + '/downloads/by-time';
      ...
      <div>
        ...
        { this.state.groups.length > 0 &&
          <table className="table table-bordered">
              ...
              <tbody>
                {this.state.groups.map(time => {
                  return (
                    <tr key={time.time}>
                      <td>{time.time}</td>
                      <td className="text-right">{time.total}</td>
                      <td className="text-right">{parseFloat(time.total / this.state.totalDownloads * 100).toFixed(2)}</td>
                    </tr>
                  )
                })}
                <tr>
